...
class BuyFinancial extends Component {
    state = {
        tableHead: [],
        tableTitle: "",
        tableItem: [],
    ...
    getFundList = () => {
        $.ajax({
            url: `${APIHOST}${APIPATH.financialList}`,
            dataType: "json",
            success: response =>{
    ...
    getBuyHead = () => {
        $.ajax({
            url: `${APIHOST}${APIPATH.shopFinancial}`,
            type: "post",
            dataType: "json",
            success: response =>{
                if(response.status === "success"){
                ...
                <React.Fragment>
                    ...
                    <div class="modal-dialog">
                        ...
                        <div class="modal-footer">
                            ...
                            <button onClick={() => {
                                $.ajax({
                                    url: `${APIHOST}${APIPATH.buyFinancial}`,
                                    type:"post",
                                    data: JSON.stringify({
                                        username: this.props.userInfo.userName,
                                        financial_amount: this.state.buyInfo.financial_amount,
                                        financial_type: this.state.buyInfo.financial_type,
                                        financial_period: this.state.buyInfo.financial_period,
                                        financial_detail_etc: this.state.buyInfo.financial_detail_etc,
                                    }),
                                    dataType: "json",
                                    success: response => {
                                        console.log(response);
...
export default connect(mapStateToProps)(BuyFinancial);