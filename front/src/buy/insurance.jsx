...
class BuyInsurance extends Component {
    ...
    getFundList = () => {
        $.ajax({
            url: `${APIHOST}${APIPATH.insuranceList}`,
            dataType: "json",
            success: response =>{
    ...
    getBuyHead = () => {
        $.ajax({
            url: `${APIHOST}${APIPATH.shopInsurance}`,
            type: "post",
            dataType: "json",
            success: response =>{
                if(response.status === "success"){
                ...
                <React.Fragment>
                    ...
                    <div class="modal-dialog">
                        ...
                        <div class="modal-footer">
                            ...
                            <button onClick={() => {
                                $.ajax({
                                    url: `${APIHOST}${APIPATH.buyInsurance}`,
                                    type:"post",
                                    data: JSON.stringify({
                                        username: this.props.userInfo.userName,
                                        insurance_amount: this.state.buyInfo.insurance_amount,
                                        insurance_type: this.state.buyInfo.insurance_type,
                                        insurance_period: this.state.buyInfo.insurance_period,
                                        insurance_policyholder: this.state.buyInfo.insurance_policyholder,
                                        insurance_insured: this.state.buyInfo.insurance_insured,
                                        insurance_detail_etc: this.state.buyInfo.insurance_detail_etc,
                                    }),
                                    dataType: "json",
                                    success: response => {
                                        console.log(response);
